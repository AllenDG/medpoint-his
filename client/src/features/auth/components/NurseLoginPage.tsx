...
const MOCK_NURSES: Record<string, { name: string; nurseId: string }> = {
  'allen@gmail.com': { name: 'Allen Walter', nurseId: 'nurse001' },
};
const MOCK_PASSWORD = 'adminNurse@123';
...
export function NurseLoginPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setCaptchaErr(false);
    const emailKey = employeeId.trim().toLowerCase();
    const nurse = MOCK_NURSES[emailKey];
    ...
    setTimeout(() => {
      setAuth(
        { id: nurse.nurseId, name: nurse.name, email: emailKey, role: 'NURSE' },
        `mock-nurse-token-${nurse.nurseId}`
      );
      setLoading(false);
      recaptchaRef.current?.reset();
      setCaptchaToken(null);
      navigate(`/nurse/${nurse.nurseId}`, { replace: true });
    }, 900);
  }
  ...
  return (
      ...
      <div className="flex-1 flex items-center justify-center p-8" style={{ background: '#F4F6F9' }}>
        <div className="w-full max-w-[420px]">
          ...
          <div className="bg-white rounded-2xl p-9" style={{ border: '1.5px solid #E4E8EF', boxShadow: '0 12px 40px rgba(18,32,86,.07)' }}>
            ...
            <p className="text-[13.5px] mb-7" style={{ color: '#64748B' }}>Sign in with your registered email to access your dashboard.</p>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button type="button" onClick={() => setShowForgot(true)}
                    className="text-[12px] font-semibold bg-transparent border-0 cursor-pointer p-0"
    ...
    {showForgot && <ForgotPasswordModal onClose={() => setShowForgot(false)} accent="#16A34A" />}
    </>
  );
}
